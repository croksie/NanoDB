...
std::any IntType::deserialize(const std::vector<uint8_t>& data)
{
    int value = 0;
    for (size_t i = 0; i < sizeof(int) && i < data.size(); ++i)
        value |= static_cast<int>(data[i]) << (8 * i);
...
std::any StringType::deserialize(const std::vector<uint8_t>& data)
{
    ...
    for (size_t i = 0; i < sizeof(size_t); ++i)
    {
        length |= static_cast<size_t>(data[i]) << (8 * i);
    }
